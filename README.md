...
5. **Brand verification**, so the Google window shows the app name and logo:
   ...
   2. Google Cloud console → **Google Auth Platform** → **Branding**: app name `DnD Companion App`, a square logo of 120 × 120 px, homepage `https://dnd.yannickmul.nl`, privacy policy `https://dnd.yannickmul.nl/privacy`, terms of service `https://dnd.yannickmul.nl/terms`, and `yannickmul.nl` under Authorized domains. Save.
...
3. **Supabase dashboard → Authentication:**
   1. **Emails → SMTP Settings:** switch on custom SMTP. Sender email `noreply@yannickmul.nl`, sender name `DnD Companion App`, host `smtp.resend.com`, port `465`, username `resend`, password: the Resend API key. Save.
   ...
   5. **Emails → Templates → Confirm signup:** subject `Confirm your DnD Companion App account`, and as the message:

      ```html
      <h2>Welcome to DnD Companion App</h2>
   ...
   6. **Emails → Templates → Reset password:** subject `Reset your DnD Companion App password`, and as the message:
